...
func runPhotoLibrarySession() {
    ...
    while let line = readLine() {
        do {
            let request = try decodeEnvelope(line)
            guard request.schemaVersion == 3,
                  request.type == .request,
                  let command = request.payload["command"]?.stringValue
            else {
                throw PhotoLibraryAdapterError.invalidRequest(
                    "Expected a JSONL v3 request envelope."
                )
            }
            switch command {
            case "discover":
            ...
            try? emit(Envelope(schemaVersion: 3, type: .error, requestID: fallbackID, payload: [
...
if command == "photo-library-authorize" {
    ...
    do {
        ...
        let message = (error as? LocalizedError)?.errorDescription ?? String(describing: error)
        try? emit(Envelope(schemaVersion: 3, type: .error, requestID: requestID, payload: [
